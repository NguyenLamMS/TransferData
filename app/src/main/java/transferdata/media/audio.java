...
public class audio extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final adapterDetail adapterDetail = new adapterDetail(audio.this,R.layout.list_item_detail,getAudio.listAudio, detail);
    ...
    Boolean setCheckAll(){
        for(int i = 0;i<getAudio.listAudio.size();i++){
            if(!getAudio.listAudio.get(i).isChecked()){
                return false;
            }
        }
        return true;
        ...
        checkBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(checkBox.isChecked()){
                    for(int i = 0;i<getAudio.listAudio.size();i++){
                        getAudio.listAudio.get(i).setChecked(true);
                    }
                }else{
                    for(int i = 0;i<getAudio.listAudio.size();i++){
                        getAudio.listAudio.get(i).setChecked(false);
                    }
                }
