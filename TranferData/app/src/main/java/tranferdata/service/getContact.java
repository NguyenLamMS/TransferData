    ...
    public static String backupContacts() {
        ...
        if(type.size() > 0) {
            if(type.indexOf("all") == -1) {
                ...
                String or = "";
                for (String s: type) {
                    if(s.indexOf("@") == -1) selectContact += or + ContactsContract.RawContacts.ACCOUNT_TYPE + " like '%"+s+"%' ";
                    else selectContact += or + ContactsContract.RawContacts.ACCOUNT_TYPE + " like '%google%' AND " + ContactsContract.RawContacts.ACCOUNT_NAME + " = '"+s+"' ";
                    or = "OR ";
        ...
        Cursor cursor = contentResolver.query(ContactsContract.Contacts.CONTENT_URI, null,selectContact, null, null);
        if (cursor != null && cursor.getCount() > 0) {
            try {
                ...
                float size = (float) fileContacts.length()/1024;
                size = (float) (size * 0.001);
                client.SIZE_ALL_ITEM[0] = (int) size;
